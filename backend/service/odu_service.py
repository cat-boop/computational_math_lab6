...
def find_best_similar(id, x0, y0, interval_length, step, eps):
    f = odu_dao.get_odu_by_id(id).function

    n = math.ceil(interval_length / step) + 1

    x = [x0 + step * i for i in range(n)]

    y_eiler = enhanced_eiler_differentiation(f, x, y0, step, eps)
    y_runge_kutta = runge_kutta_differentiation(f, x, y0, step, eps)

    try:
        y_miln = miln_differentiation(f, x, y0, step, eps)
    except ValueError:
        y_miln = None
    ...
    return OduResponseDto(enhanced_eiler=enhanced_eiler, runge_kutta=runge_kutta, miln=miln, x_range=[x_min, x_max],
                          y_range=[y_min, y_max])
